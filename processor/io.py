...
class IO():
    ...
    def create_filename(self, save_dir, args, isbest=False, num_epochs=-1):
        """
        Args:
        ...
        """
        filename = os.path.join(save_dir, self.gen_prefix(args))
    ...
    def load_ckpt(self, args, isbest=False):
        '''Load a pre-trained pytorch model from checkpoint.
        '''
        print("loading model")
        filename = self.create_filename(args.ckptdir, args, isbest)
        print(filename)
    ...
    def gpu(self):
        ...
        if self.arg.use_gpu and len(self.gpus) > 1:
            self.model = nn.DataParallel(self.model, device_ids=[1,7])
